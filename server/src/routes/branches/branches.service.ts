...
export const createBranchService = async (name: string) => {
  // ── FIX #17: Use ValidationError so errorHandler returns 400 ─────────────
  if (!name?.trim()) throw new ValidationError('Branch name is required')
  if (name.trim().length > 255) throw new ValidationError('Branch name too long')

  // ── FIX #16: Check duplicate, return 409 instead of 500 ─────────────────
  const existing = await pool.query(
    `SELECT branch_id FROM branches WHERE LOWER(name) = LOWER($1)`, [name.trim()]
  )
  if (existing.rows.length > 0) throw new AppError('A branch with this name already exists', 409)
  ...
  try {
    ...
    // ── FIX #16: Catch DB unique constraint → 409 ────────────────────────
    if (err.code === '23505') {
      throw new AppError('A branch with this name already exists', 409)
    }
    throw err
  }
}
...
export const updateBranchService = async (branch_id: number, name: string) => {
  ...
  if (!res.rows[0]) throw new NotFoundError('Branch not found')
  await cacheDel(CK.BRANCHES_ALL)
  return res.rows[0]
}
...
export const deleteBranchService = async (branch_id: number) => {
  ...
  // ── FIX #18: Use NotFoundError so errorHandler returns 404 ─────────────
  if (!res.rows[0]) throw new NotFoundError('Branch not found')
  await cacheDel(CK.BRANCHES_ALL)
  return res.rows[0]
}
...
export const createTeamService = async (branch_id: number, name: string) => {
  ...
  if (name.trim().length > 255) throw new ValidationError('Team name too long')
  ...
  if (existing.rows.length > 0) throw new AppError('A team with this name already exists in this branch', 409)
  ...
  try {
    ...
    if (err.code === '23505') {
      throw new AppError('A team with this name already exists in this branch', 409)
    }
    throw err
  }
}
...
export const updateTeamService = async (team_id: number, name: string) => {
  ...
  if (!team.rows[0]) throw new NotFoundError('Team not found')
...
export const deleteTeamService = async (team_id: number) => {
  ...
  if (!res.rows[0]) throw new NotFoundError('Team not found')
  await cacheDel(CK.BRANCHES_ALL)
  return res.rows[0]
}